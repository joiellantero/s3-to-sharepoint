...
def get_sharepoint_auth_token(grant_type, client_id, client_secret, resource):
    try:
        # perform a get request to SharePoint to obtain the jwt
...
def lambda_handler(event, context):
    ...
    filename, bucket_name = get_s3_file(event)

    token = get_sharepoint_auth_token(grant_type, client_id, client_secret, resource)

    # identifies which bucket is the file coming from and uploads it to its corresponding folder in sharepoint
    upload_to_sharepoint(token, filename, 'MyFolder')
